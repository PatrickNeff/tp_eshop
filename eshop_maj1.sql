...
--
-- Client: localhost
-- Généré le: Mar 09 Juin 2015 à 16:47
...
CREATE TABLE IF NOT EXISTS `adresse` (
  ...
  `type_adresse` varchar(40) COLLATE utf8_bin NOT NULL,
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='table des adresses de nos client' AUTO_INCREMENT=15 ;

--
-- Contenu de la table `adresse`
--

INSERT INTO `adresse` (`id`, `client_id`, `type_adresse`, `street`, `zipcode`, `city`, `country`) VALUES
(1, 2, '', '', 0, '', ''),
(3, 5, '', '', 0, '', ''),
(5, 76, 'principale', 'fesfesgsgss', 67000, 'strasbourg', 'France'),
(6, 76, 'livraison', 'rghgzefafeszfsef', 75000, 'paris', 'france'),
(7, 0, 'principale', 'fesfesgsgss', 67000, 'strasbourg', 'France'),
(8, 0, 'livraison', 'rghgzefafeszfsef', 75000, 'paris', 'france'),
(9, 3, 'principale', '', 0, '', ''),
(10, 3, 'livraison', '', 0, '', ''),
(11, 77, 'principale', 'fesfesgsgss', 0, 'paris', 'France'),
(12, 77, 'livraison', 'rghgzefafeszfsef', 0, 'paris', ''),
(13, 1, 'principale', '', 0, '', ''),
(14, 1, 'livraison', '', 0, '', '');
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='catégorie des produits de notre catalogue' AUTO_INCREMENT=7 ;
...
INSERT INTO `client` (`id`, `civility`, `pseudo`, `firstname`, `lastname`, `password`, `email`, `phone`, `groupe_id`, `permission_id`, `time_register`, `time_update`) VALUES
(76, 'M', 'fred', 'fred', 'fred', '$2y$10$D3Ds8zhPmcDc8BMpu66FLe74x4ZVrg4RQWDU9Wf8QTKIYBQmIDDSW', 'test2@test.fr', 0, 0, 3, '2015-06-08 12:17:48', '2015-06-05 14:48:47'),
(77, 'M', 'admin', 'fesfsegfsgegs', 'fesefsfes f', '$2y$10$2mFOiXvxr8P.Hjb63AuRBuMtGuDhPZnZiue5iq0hAhusZGHv2zJPi', 'test@test.fr', 388000000, 1, 3, '2015-06-09 09:02:57', '2015-06-05 09:14:39');
...
CREATE TABLE IF NOT EXISTS `note` (
  ...
  `time_create` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'timestamp de mise à jour de l''avis',
  PRIMARY KEY (`id`)
...
(1, 'reines des reinettes', 'une des variétés préférées des amateurs de pommes', 1, 2, 'images/pomme-reine-des-reinettes.jpg', 'sud de la France', 200, 0, 0),
(2, 'granny smith', 'pomme avec un goût assez acidulé, très fermes, croquantes, juteuses, excellentes à cuire ou à croquer', 1, 2, 'images/pomme-granny-smith.jpg', 'sud de la France', 150, 0, 0),
(4, 'golden', 'pomme la plus cultivée puisqu''elle représente plus d''un tiers de la production nationale', 1, 2, 'images/pomme-golden.jpg', 'france', 180, 0, 0),
(5, 'boskoop', 'pomme à chair est ferme, finement acidulée et légèrement jaune et sucrée', 1, 1, 'images/pomme-boskoop.jpg', 'sais pas', 250, 0, 0),
(6, 'jonagold', 'pomme à à peau moyennement fine, grosse et parfumée', 1, 1, 'images/pomme-jonagold.jpg', 'allemagne', 240, 0, 0),
(7, 'antares', 'pomme d’aspect rustique obtenue dans les années 1990', 1, 2, 'images/pomme-antares.jpg', 'france', 10, 0, 0),
(8, 'conference', 'poire à couteau allongée en forme de bouteille, de taille moyenne.', 2, 2, 'images/poire-conference.jpg', 'portugal', 50, 0, 0),
(9, 'rochas', 'poire portugaise découverte en 1836', 2, 2, 'images/poire-rochas.jpg', 'portugal', 5, 0, 0),
(10, 'williams', 'poire assez grosse, juteuse, sucrée avec une pointe d’acidité. Sa chair blanche non granuleuse est très aromatique.', 2, 2, 'images/poire-williams.jpg', 'france', 0, 0, 0),
(11, 'clementine', 'ne clémentine se divise généralement en une dizaine de quartiers. Un quartier est parfois appelé une cuisse, un grain ou un taillon', 3, 2, 'images/clementine.jpg', 'maroc', 50, 0, 0),
(12, 'clemenvilla', 'mandarine sans pépin', 3, 2, 'images/clemenvilla.jpg', 'maroc', 100, 0, 0),
(13, 'minneola', 'fruit orangé, au zeste brillant et granuleux, juteux, la pulpe est d''une couleur oranger saturée', 3, 2, 'images/minneola.jpg', 'france', 50, 0, 0),
(14, 'orange', 'fruit de couleur orange qui possède une peau épaisse et assez rugueuse', 3, 2, 'images/orange.jpg', 'maroc', 20, 0, 0),
(15, 'fraises', 'faux fruit : réceptacle charnu sur lequel sont disposés régulièrement des akènes dans des alvéoles plus ou moins profondes, la fraise étant donc un polyakène.', 4, 5, 'images/fraise.jpg', 'sud de la france', 20, 0, 0),
(16, 'framboises', 'fruit issu de la transformation de la quarantaine de minuscules carpelles d''une seule et même fleur, qui se transforment en drupéoles semi-soudées', 4, 4, 'images/framboises.jpg', 'sud de la france', 20, 0, 0),
(17, 'epinards', 'demande à popeye!', 5, 2, 'images/epinard.jpg', 'alsace', 20, 0, 0),
(18, 'petits-pois', 'des trucs verts tout petits', 5, 2, 'images/petits-pois.jpg', 'alsace', 20, 0, 0),
(19, 'haricots verts', 'Les haricots verts cultivés sont des variétés de Phaseolus vulgaris, le haricot commun.', 5, 2, 'images/haricots-verts.jpg', 'alsace', 5, 0, 0),
(20, 'choux fleurs', 'Les variétés de choux-fleur fleurissent toute l''année. On parle donc de chou-fleur de printemps, d''été, d''automne et d''hiver.', 6, 2, 'images/choufleur.jpg', 'france', 5, 0, 0),
(21, 'brocoli', 'Choux d''Italie défini aussi comme "le petit rejeton que le tronc d''un vieux chou pousse après l''hiver". ', 6, 1, 'images/brocoli.jpg', 'france', 5, 0, 0),
(22, 'mangue', 'Délicieusement parfumée, juteuse et tendre, la mangue est un fruit charnu.', 7, 2, 'images/mangue.jpg', 'îles paradisiaques', 5, 0, 0),
(23, 'kiwi', 'fruits de plusieurs espèces de lianes du genre Actinidia, famille des Actinidiaceae', 8, 6, 'images/kiwi.jpg', 'îles paradisiaques', 10, 0, 0),
(24, 'ananas', 'tu connais pas?', 8, 3, 'images/ananas', 'îles paradisiaques', 20, 0, 0),
(25, 'noisettes', 'coquille akène doté d’un péricarpe ligneux appelé involucre ou quelquefois, par erreur, écale et renfermant une seule2 graine (l''amande) qui occupe normalement3 toute la cavité interne du péricarpe', 9, 5, 'images/noisettes.jpg', 'mon jardin :-)', 25, 0, 0);
